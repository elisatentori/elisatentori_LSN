...
void initialize() {
    ...
    //Read seed for random numbers     // Non me ne voglia, però ho usato il suo generatore random anzichè TRandom3 di root :(
    int p1, p2;
...
// The external potential. You can modify this function but don't forget to modify its first and second derivatives too !
double external_potential(double val) {
	//double k_elastic = 1;
	//return k_elastic*val*val/2.0;   // V_e(x) = 1/2 k x^2
    return val*val*(val*val-5.0/2.0);   // V(x) = x^4 - 5/2 x^2
}



double external_potential_prime(double val) {
    return val*(4.0*val*val-5.0);    // V'(x) = 4 x^3 - 5 x
}



double external_potential_second(double val) {
    return 12.0*val*val-5.0;   // V''(x) = 12 x^2 - 5
}


                    /* *** MODIFICA Psi(x) ***  */

// The same applies to the variational Wave Function... You can modify this function but don't forget to modify its second derivative below!
double variationalWaveFunction(double v) {
	//return 1.0;   // Psi = 1
    return exp(-1*(v-mu)*(v-mu)/(2*sigma*sigma))+exp(-1*(v+mu)*(v+mu)/(2*sigma*sigma));   // Psi nostra
}
...
double variationalWaveFunction_second(double v) {
    ...
    double p;      // Psi'' nostra
    double exp1 = exp( -1*(v-mu)*(v-mu)/(2*sigma*sigma) );
    double exp2 = exp( -1*(v+mu)*(v+mu)/(2*sigma*sigma) );
    p = -exp1-exp2 + pow(( (v-mu)/sigma ),2.) * exp1 + pow( ((v+mu)/sigma) ,2.) * exp2;
...
void finalizePotentialEstimator() {
	ofstream out("out.potential.dat");
	for(int i=0;i<timeslices;i++) {
...
void finalizeHistogram() {
	ofstream out("out.probability.dat");
