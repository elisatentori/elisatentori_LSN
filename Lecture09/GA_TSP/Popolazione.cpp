...
void Popolazione :: AddCromo(Cromosoma chr) {
	pop.push_back(chr);
    cout << " nstep " << nstep <<  "  ncity " << ncity << "   npop " << npop << endl;
    cout << " m_mut " << m_mut << "   m_cross " << m_cross << endl;
    return;
}



// Gli passi il primo cromo, fa "npop" permutazioni di esso e aggiunge i cromosomi
// permutati alla popolazione
...
// Mutazione: muto due genitori per generare due nuovi figli
// che vanno a sostituire i due cromosomi di coda della popolazione
...
void Popolazione :: Crossover(Random &rand) {
    _rnd=&rand;
    // Genitore 1 - Genitore 2   Genitore 1 - Genitore 2  Genitore 1 - Genitore 2
    int parent[ncity],parent2[ncity];
    int k,l;
    
    //copio i due cromosomi figli, che attualmente si trovano in coda alla Popolazione
    ...
    if(_rnd->Rannyu()<=m_cross){
        int n = (int)(_rnd->Rannyu(1,ncity-1));  // dove tagliare la coda del cromosoma
...
//seleziona un cromosoma i secondo la probabilità p(L[i]) = 1 - L[i]/L_tot
...
// Seleziona un cromosoma secondo la probabilità r^p
