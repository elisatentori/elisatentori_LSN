...
void Input(void) {
    ...
    cout << "Interatomic potential v(r) = 4 * [(1/r)^12 - (1/r)^6]" << endl << endl;  // tutto già riscalato
    ...
    ReadInput >> delta;  // per l'incremento

    ReadInput >> nblk;

    ReadInput >> nstep;  // nstep per blocco
    
    ReadInput >> simul;  // se ==1 equilibrazione, se no simulazione
    
    if(simul==1) cout <<  endl << "Prepearing system for equilibration" << endl << endl;
...
void Move(void) {    // catena di markov
    int o;
    double p, energy_old, energy_new;   // p = alfa
...
void Measure() {
    int bin;
    double v = 0.0, w = 0.0;  // potenziale e viriale inizializzati
    double vij, wij;
    double dx, dy, dz, dr;
    ...
    if((simul==1)&(equil==1)){
        ...
        Epot.open("output_instant_epot.dat",ios::app);
        Epot << walker[iv]/(double)npart + vtail << endl;
        Epot.close();
        
        Pres.open("output_instant_pres.dat",ios::app);
